                ...
                <div>
                    <div className=' bg-white shadow lg:w-[581px] lg:ml-0 lg:h-14 rounded-full  relative top-28 md:w-[581px] md:h-14 md:ml-9  md:-mt-0 sm:ml-5 sm:w-[85%] sm:h-12 sm:-mt-28'>
                        <div className=' lg:flex justify-between md:flex sm:block'>
                            ...
                            <button className=' rounded-full bg-black  lg:w-40 lg:h-14 text-white font-semibold lg:mt-0 md:mt-0 lg:relative md:w-40 md:h-14 md:relative sm:absolute  sm:mt-7 sm:h-12 sm:w-[100%] '>Search Now</button>
                        </div>
                    </div>
                </div>
                        ...
                        Record.map((card, index) => (
                            <div key={card.id} >
                                <div className=' lg:bg-white lg:w-[261px] lg:h-[295px] lg:pb-0 rounded-xl shadow md:w-[90%] md:h-[100%] md:pb-12 ml-8 lg:ml-0  md:mt-20 sm:w-[80%] sm:pb-6 '>
                                    <img className=' md:w-[100%]  sm:w-[100%] ' src={card.image} alt="" />
                                    <div className=' flex justify-between pt-2'>
                                      <h3 className=' pl-4  lg:text-sm font-bold md:text-3xl'> {card.name}</h3>
                                       <p className='text-[#8A0B0B] lg:text-sm pr-4 font-bold  md:text-3xl'>{card.price}</p>
                                    </div>
                                    <button className=' bg-[#8A0B0B] lg:w-20 lg:h-8 lg:text-sm text-white ml-4 lg:mt-2 font-semibold md:w-44 md:h-14 md:mt-5 md:text-xl sm:w-[32%] sm:h-8'>Add Cart</button>
                                </div>
                            </div>
                        ))
                    }
